...
{#if $isOpenStore || !finishedAnimation}
	<div
		...
		class="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 pointer-events-auto"
		data-aria-hidden="true"
		aria-hidden="true"
	/>
	<div
		on:animationend={onAnimationend}
		use:clickOutside={() => ($isOpenStore = false)}
		role="alertdialog"
		data-state={$isOpenStore ? 'open' : 'closed'}
		class={cn(
			className,
