<script lang="ts">
	...
	let isActiveStore: Writable<boolean> = writable(false);
	let init: InitFunctionType = getContext('tabs-list');
</script>

<button
	use:init={{ store: isActiveStore, value, initResult }}
	on:click={initResult.toggleItem}
	type="button"
	role="tab"
	aria-selected="true"
	data-state={$isActiveStore ? 'active' : 'inactive'}
	tabindex={$isActiveStore ? 0 : -1}
